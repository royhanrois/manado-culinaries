...
if (isset($_POST["res_id"])) {
    ...
    echo "Error: 'res_id' not found in the request.";
    exit;
}

$resto = query("SELECT * FROM restoran WHERE res_id = $resId")[0];
...
<body>
  <div class="container">
    ...
    <form id="profileForm" onsubmit="updateResto(event)" method="post" action="" enctype="multipart/form-data">
      ...
      <input type="hidden" name="res_id" value="<?= $resto["res_id"]; ?>">
      <input type="hidden" name="user_id" value="<?= $resto["user_id"]; ?>">
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.trim() === "success") {
          ...
          loadPage("adminRestaurant"); // Reload the content on the restaurant page after successful creation
        } else {
            console.log("Error response:", response);
          alert("Error submitting restaurant details."+ response); // Error message
        }
      },
      error: function () {
